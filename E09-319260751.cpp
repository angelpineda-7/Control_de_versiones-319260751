...
int main()
{
	...
	dragonCuerpo = Model();
	dragonCuerpo.LoadModel("Models/dragonCuerpoCabeza.obj");
	alaDerecha = Model();
	...
	float tiempoTranscurridoDecenas = 0.0f;
	int unodos = 1;
	...
	while (!mainWindow.getShouldClose())
	{
		...
		if (tiempo <= tiempoTranscurrido) {
			...
			tiempoTranscurridoDecenas += tiempoTranscurrido;
		...
		if (tiempoTranscurridoDecenas <= (4 * tiempo) && unodos == 1) {
			Numero1Texture.UseTexture();
		}
		else if (unodos == 1) {
			Numero2Texture.UseTexture();
			unodos = 2;
			tiempoTranscurridoDecenas = 0.0f;
		}
		else if (tiempoTranscurridoDecenas <= (4 * tiempo) && unodos == 2) {
			Numero2Texture.UseTexture();
		}
		else if (unodos == 2) {
			Numero1Texture.UseTexture();
