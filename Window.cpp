...
void Window::ManejaTeclado(GLFWwindow* window, int key, int code, int action, int mode)
{
	...
	if (key == GLFW_KEY_F)
	{
		theWindow->adelante += 1.0;
		theWindow->sigue -= 5.0;
		theWindow->isAvanzando = 1;
	}
	if (key == GLFW_KEY_G) 
	{
		theWindow->atras -= 1.0;
		theWindow->retrocede += 5.0;
		theWindow->isAvanzando = 0;
	}
	if (key == GLFW_KEY_H)
	{
		if (theWindow->cofre < 45.0 && theWindow->c == 0)
		{
			theWindow->cofre += 5.0;
		}
		...
		else if (theWindow->cofre > 0.0 && theWindow->c == 1)
		{
			theWindow->cofre -= 5.0;
		}
		else
